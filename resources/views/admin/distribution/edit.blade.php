...
@section('content')
    <div class="px-4 sm:px-0">
        <div class="admin-hero">
            ...
            <div>
                <h1 class="admin-hero-title">{{ __('admin.distribution.edit_heading') }}</h1>
                <p class="admin-hero-sub">{{ __('admin.distribution.edit_subtitle') }}</p>
            </div>
        </div>
                ...
                <form method="POST" action="{{ route('admin.distribution.update', ['channelId' => (int) $channel->id]) }}" class="space-y-6">
                    ...
                    @elseif ($channel->isShopifyBlog())
                        <div class="rounded-lg border border-emerald-100 bg-emerald-50 p-5">
                            ...
                            @php $shopifyAuthMode = old('shopify_auth_mode', $shopifyConfig['shopify_auth_mode']); @endphp
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                <div>
                                ...
                                <div>
                                    ...
                                    <input id="shopify_api_version" name="shopify_api_version" type="text" value="{{ old('shopify_api_version', $shopifyConfig['shopify_api_version']) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="2025-10">
                            ...
                            <div data-shopify-auth="client_credentials" @class(['mt-6 grid grid-cols-1 gap-6 md:grid-cols-2', 'hidden' => $shopifyAuthMode !== 'client_credentials'])>
                                <div>
                                    ...
                                    <input id="shopify_client_id" name="shopify_client_id" type="text" value="{{ old('shopify_client_id', $shopifyConfig['shopify_client_id']) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" autocomplete="off">
                                </div>
                                <div>
                            ...
                            <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
                                <div>
                                    ...
                                    <select id="shopify_blog_strategy" name="shopify_blog_strategy" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <option value="first_blog" @selected(old('shopify_blog_strategy', $shopifyConfig['shopify_blog_strategy']) === 'first_blog')>{{ __('admin.distribution.shopify.blog_first') }}</option>
                                        <option value="match_handle" @selected(old('shopify_blog_strategy', $shopifyConfig['shopify_blog_strategy']) === 'match_handle')>{{ __('admin.distribution.shopify.blog_match_handle') }}</option>
                                        <option value="fixed" @selected(old('shopify_blog_strategy', $shopifyConfig['shopify_blog_strategy']) === 'fixed')>{{ __('admin.distribution.shopify.blog_fixed') }}</option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input id="shopify_blog_handle" name="shopify_blog_handle" type="text" value="{{ old('shopify_blog_handle', $shopifyConfig['shopify_blog_handle']) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="news">
                                </div>
                                <div>
                            ...
                            <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
                                <div>
                                    ...
                                    <input id="shopify_author" name="shopify_author" type="text" value="{{ old('shopify_author', $shopifyConfig['shopify_author']) }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('admin.distribution.shopify.author_placeholder') }}">
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <select id="shopify_summary_strategy" name="shopify_summary_strategy" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <option value="excerpt" @selected(old('shopify_summary_strategy', $shopifyConfig['shopify_summary_strategy']) === 'excerpt')>{{ __('admin.distribution.shopify.summary_excerpt') }}</option>
                                        <option value="meta_description" @selected(old('shopify_summary_strategy', $shopifyConfig['shopify_summary_strategy']) === 'meta_description')>{{ __('admin.distribution.shopify.summary_meta_description') }}</option>
                                        <option value="disabled" @selected(old('shopify_summary_strategy', $shopifyConfig['shopify_summary_strategy']) === 'disabled')>{{ __('admin.distribution.shopify.summary_disabled') }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                                <div>
                                    ...
                                    <select id="shopify_image_strategy" name="shopify_image_strategy" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <option value="hero_as_featured" @selected(old('shopify_image_strategy', $shopifyConfig['shopify_image_strategy']) === 'hero_as_featured')>{{ __('admin.distribution.shopify.image_hero_as_featured') }}</option>
                                        <option value="disabled" @selected(old('shopify_image_strategy', $shopifyConfig['shopify_image_strategy']) === 'disabled')>{{ __('admin.distribution.shopify.image_disabled') }}</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <label class="mt-2 inline-flex items-center gap-2 text-sm text-gray-700">
                                        ...
                                        <input type="checkbox" name="shopify_published" value="1" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" @checked(filter_var(old('shopify_published', $shopifyConfig['shopify_published']), FILTER_VALIDATE_BOOLEAN))>
                            ...
                            <div class="mt-6 border-t border-gray-200 pt-5">
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm leading-6 text-gray-600">管理这个 TavixGEO Agent 目标站点的首页模块、样式、轮播与默认站同步关系。</p>
                                    </div>
                                ...
                                <div class="mt-4 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-900">
                                    支持模块：{{ implode('、', $supportedFrontendModules) }}。WordPress REST 和 Generic API 只透传字段，不保证渲染 TavixGEO 模块。
                                </div>
                    ...
                    @if (! $channel->isGeoFlowAgent())
                        <div class="rounded-lg border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-6 text-amber-900">
                            WordPress REST 和 Generic API 只作为外部分发渠道处理，可接收字段透传，不保证渲染 TavixGEO 首页模块、轮播或主题映射。
                        </div>
                    @endif
    ...
    <template id="channel-text-ad-link-template">
        <div class="rounded-md border border-gray-200 bg-gray-50 p-3" data-channel-text-ad-link>
            ...
            <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-4">
                <div class="lg:col-span-2">
                    <label class="block text-xs font-medium text-gray-700">{{ __('admin.site_settings.ads.text_field_tracking') }}</label>
                    <input type="text" name="article_text_ad_policy[__PLACEMENT__][custom_modules][__MODULE_INDEX__][links][__LINK_INDEX__][tracking_param]" value="utm_source=tavixgeo&utm_medium=article_text_ad" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <label class="mt-5 inline-flex items-center gap-2 text-sm font-medium text-gray-700">
